...
double AltFun(Wisdom wis) {
    // Длина строки.
    double num = strlen(wis.content);
    // Количество знаков препинания.
    double num1 = 0;
    for (int i = 0; i < strlen(wis.content); i++) {
        if (strchr(punctuation, wis.content[i]) != NULL) {
            num1++;
        }
    }
    return num / num1;
...
void File_Inp(char* output_file){
    ...
    for (int i = 0; i < size; i++) {
        ...
        {
            case APHORISM:
                fprintf(fpp, arr[i].content);
                fprintf(fpp, "\n");
                fprintf(fpp, arr[i].aph->author);
                break;
            case PROVERB:
                fprintf(fpp, arr[i].content);
                fprintf(fpp, "\n");
                fprintf(fpp, arr[i].prov->origin_country);
                break;
            case RIDDLE:
                fprintf(fpp, arr[i].content);
                fprintf(fpp, "\n");
                fprintf(fpp, arr[i].rid->answer);
                break;
        }
        fprintf(fpp, "\n");
    }
